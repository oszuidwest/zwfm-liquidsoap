...
LIQUIDSOAP_LIB_SERVER_COMMANDS_URL="${GITHUB_BASE}/conf/lib/server_commands.liq"
...
if ! file_download -m "${LIQUIDSOAP_LIB_DIR}" "Liquidsoap library files" \
  ...
  "${LIQUIDSOAP_LIB_DAB_OUTPUT_URL}|dab_output.liq" \
  "${LIQUIDSOAP_LIB_SERVER_COMMANDS_URL}|server_commands.liq"; then
  exit 1
fi
...
echo -e ""
echo -e "${YELLOW}To control silence detection at runtime:${NC}"
echo -e "  Set SERVER_SOCKET_ENABLED=true in .env, then use:"
echo -e "  ${CYAN}docker exec -i liquidsoap socat - UNIX-CONNECT:/tmp/liquidsoap.sock${NC}"
echo -e "  Commands: silence.enable, silence.disable, silence.status"
